...
- **`/people/current` is a single flat grid, no section headings** — the per-card type badge carries the role. Ordering priority is set by the `TYPE_ORDER` array in that page (default: `Staff → PostDoc → PhD → MPhil`), then start year desc, then name.
...
- 6 top-level tabs: Home · News · Research · People · Resources · Contact.
- Research / People / Resources have dropdowns. **News is a plain leaf** → `/news` (formerly "Community"; Events was retired — see `struct/Events.md`). Contact is a primary-blue CTA leaf at the right end of nav.
...
- **In-page section nav** (`SectionNav.tsx`) — sticky `top-[80px]` jump bar (no visible label), placed right after `<SubPageHero>` on pages with ≥2 anchored sections (currently Research→Impact). Pass `items={[{id,label}]}`; each section element needs a matching `id` + `scroll-mt-[132px]` (= 80px header + ~52px bar). Self-renders nothing for <2 items. Scroll-spy via IntersectionObserver with an 800ms click-lock to stop highlight flicker during smooth scroll.
- **Chronological archive** (`Chronology.tsx`) — the layout for time-stamped collections (`/news`, Research→Publications "All papers", Research→Projects). Left: sticky year-bucketed index that scroll-spies the entry in view and keeps that year's group expanded (collapsible; hidden under `lg`). Right: the entries under year headings. Pass `entries={[{id, year, label, node}]}` (already ordered within each year — buckets sort newest-first); `emptyState` renders when there are none. Featured publications stay a separate top section — they're not time-stamped.
